...
export class FruitCountPage implements OnInit, AfterViewInit {
  ...
  private scoreThreshold: number = 0.75;
  ...
  constructor(private route: ActivatedRoute, private navCtrl: NavController, private loadingController: LoadingController, private toastController: ToastController) {}

  // Getters and Setters for _confidenceSlider
  ...
  private async initializeEventListeners() {
    ...
    this.calculateScoreThreshold(5);
  ...
  public async handleImageUpload(input: any) {
    ...
    // Determine if the input is a filename or a file event
    ...
    this.log(`Selected image file: ${this.imageFilename}`, 2);
    ...
    try {
      ...
      img.onload = async () => {
        ...
        try {
          await this.showLoading('Detectando frutos...');
   ...
   */
  private async drawEllipses(): Promise<void> {
    if (this.predictBoxes && this.predictScores && this.ctx) {
      ...
      if (total_before == 0) {	
         this.presentToast(`Se han detectado ${this.totalSelectedObjects} ${this.fruitName}s.`, 'bottom');
      ...
      if (this.totalSelectedObjects === 0) {
        ...
        for (const { centerX, centerY, radiusX, radiusY, index, score } of ellipses) {
          ...
          if (score > this.scoreThreshold) {
            ...
            this.drawBox(`${this.totalSelectedObjects}`, 'top-left');

            if (this.fruitSampleFilename === `./assets/test-dataset/${this.imageFilename}`) {
              this.drawBox('Imagen de Ejemplo', 'middle-center');
            }
            this.drawBox(this.fruitSampleFilename, 'top-center', "small");
          }
        }
      }
  ...
  private drawBox(
    ...
    // Dibujar el cuadro semi-transparente
    ...
    this.ctx!.closePath();
    this.log(`Drawn box with text: "${text}" at position ${position} (${boxX},${boxY})`, 2);
  }
  ...
  private async drawRoundedImage(canvasOrCtx: HTMLCanvasElement | CanvasRenderingContext2D, imageInput: string | HTMLImageElement) {
    ...
    if (img.complete) {
      // If the image is already loaded, draw directly
  ...
  /**
   * Auxiliary function to draw a rounded image on the canvas.
   * @param ctx The canvas rendering context.
   * @param img The image to draw.
   * @param canvas The canvas element.
   */
  private drawRoundedImageOnCanvas(ctx: CanvasRenderingContext2D, img: HTMLImageElement, canvas: HTMLCanvasElement) {
    const borderSize = 9;
    ...
    // Draw the rounded rectangle and the image
    this.drawRoundedRect(ctx, 0, 0, canvas.width, canvas.height, borderSize);
    ctx.clip();
  ...
  private adjustCanvasDimensions(img: HTMLImageElement) {
    ...
    // Display size (given by CSS width=100% height=auto)
    const canvasWidth = window.innerWidth*0.90;
    this.canvas!.style.width = `${canvasWidth}px`;
    this.canvas!.style.height = `${canvasWidth / ratio}px`;

    // Reset any previous scaling of the context
   ...
   */
  resetSensitivity() {
    if (this.confidenceSlider !== 5) {
      this.confidenceSlider=5;
  ...
  private calculateScoreThreshold(newThreshold: number) {
    const lookupTable: { [key: number]: { scoreThreshold: number, iou: number } } = {
      1: { scoreThreshold: 0.85, iou: 0.20 },
      2: { scoreThreshold: 0.70, iou: 0.21 },
      3: { scoreThreshold: 0.60, iou: 0.25 },
      4: { scoreThreshold: 0.50, iou: 0.26 },
      5: { scoreThreshold: 0.35, iou: 0.27 },
      6: { scoreThreshold: 0.30, iou: 0.28 },
      7: { scoreThreshold: 0.25, iou: 0.29 },
      8: { scoreThreshold: 0.20, iou: 0.30 },
      9: { scoreThreshold: 0.15, iou: 0.31 },
      10: { scoreThreshold: 0.05, iou: 0.35 }
    };
    ...
    if (newThreshold >= 1 && newThreshold <= 10) {
      ...
      console.error('Invalid newThreshold value. Must be between 1 and 10.');
    }
  }
  
  //////////////////////////////////////////////////////////////////////////////////////////////

  public async shareCanvasImage(): Promise<void> {
    // Convert the canvas to a data URL
    const dataUrl = this.canvas!.toDataURL('image/jpeg', 0.9);

    if (Capacitor.getPlatform() === 'web') {
        // If on the web, download the image instead of sharing
        ...
        try {
            ...
            const savedFile = await Filesystem.writeFile({
                path: fileName,
                data: base64Data,
                directory: Directory.Cache // Save to cache directory
            });

            // Get the file URI
            const fileUri = savedFile.uri;

            // Share the file using the Capacitor Share plugin
            await Share.share({
