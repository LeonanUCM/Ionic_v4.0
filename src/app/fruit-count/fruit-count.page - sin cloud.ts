...
export class FruitCountPage implements OnInit, AfterViewInit {
  ...
  public async handleImageUpload(input: any) {
    ...
    // Get reference to the nextButton
    ...
    const processFile = async (index: number) => {
      ...
      try {
        ...
        img.onload = async () => {
          ...
          try {
            if (this.numImages > 1) {
              await this.showLoading(`Analisando imagen ${index+1}/${this.numImages}...`);
            } else {
              await this.showLoading('Analisando imagen...');
            }
  ...
  public async shareCanvasImage(platform: string = "auto"): Promise<void> {
    ...
    if (platform == "web" || platform == "auto" && Capacitor.getPlatform() === 'web') {
